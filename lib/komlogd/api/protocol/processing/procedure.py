...
async def initialize_transfer_method(session, method):
    initialized={metric.uri:False for metric in method.metrics}
    ...
    for ts in tss:
        ...
        if len(items)>1:
            ...
            for item in items:
                ...
                session._metrics_store.store(metric=item.metric, ts=ts, content=item.data)
            ...
            session._metrics_store.store(metric=items[0].metric, ts=ts, content=items[0].data)
